...
php "$WP_SYNC_REPOSITORY_PATH"/srdb.cli.php -h "$LOCAL_DB_HOST" -P "$LOCAL_DB_PORT" -u "$LOCAL_DB_USER" -p "$LOCAL_DB_PASSWORD" -n "$LOCAL_DB_NAME" -s "https://${MUTUAL_LINK_BLOG_STG_DOMAIN}" -r "http://${MUTUAL_LINK_BLOG_LOCAL_DOMAIN}"
printf "【完了】\n\n"
